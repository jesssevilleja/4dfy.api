...
export class QuestionsService {
  ...
  findAll({ page, first }: GetQuestionsArgs) {
  ...
  //   if (product_id) {
  //     data = data.filter((p) => p.product_id === String(product_id));
  //   }
  ...
  findOne(id: string) {
    return this.questions.find((question) => question.id === String(id));
  }

  update(id: string, updateQuestionInput: UpdateQuestionInput) {
    const question = this.questions.find((q) => q.id === String(id));
