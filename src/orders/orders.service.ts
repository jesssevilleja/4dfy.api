...
const settings = plainToClass(Settings, settingJson);
...
export class OrdersService {
  ...
  getOrders({
    ...
    if (shop_id) {
      data = this.orders?.filter((p) => p?.shop?.id === String(shop_id));
    }
  ...
  getOrder({ id, tracking_number }: GetOrderArgs): Order {
    ...
    if (id) {
      parentOrder = this.orders.find((p) => p.id === String(id));
    } else {
  ...
  async getOrderByIdOrTrackingNumber(id: string): Promise<Order> {
    try {
      return (
        this.orders.find(
          (o: Order) => o.id === String(id) || String(o.tracking_number) === id,
        ) ?? this.orders[0]
      );
    } catch (error) {
      console.log(error);
  ...
  async processPaymentIntent(
    ...
    const {
      ...
      order,
      setting.options.defaultPaymentGateway,
    );
  ...
  async savePaymentIntent(order: Order, paymentGateway?: string): Promise<any> {
    const me = await this.authService.findUserById(order.customer_id);
