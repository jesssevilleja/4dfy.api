...
export class CouponsService {
  ...
  getCoupons({ first, page, shop_id, search, sortedBy }: GetCouponsArgs) {
    ...
    if (shop_id) {
      data = this.coupons.filter((p) => String(p.shop_id) === String(shop_id));
    }
  ...
  getCoupon({ id, code }: GetCouponArgs): Coupon {
    if (id) {
      return this.coupons.find((p) => p.id === String(id));
    }
  ...
  approveCoupon(id: number) {
    const coupon = this.coupons.find((s) =>String(s.id) === String(id));
