...
export class PaymentMethodService {
  ...
  private setting: Settings = this.settingService.getSettings();
  ...
  async create(createPaymentMethodDto: AddNewCardInput) {
    try {
      ...
      const paymentGateway: string = this.setting.options.defaultPaymentGateway;
  ...
  findOne(id: string) {
    return this.paymentMethods.find((pm: Card) => pm.id === String(id));
  }

  update(id: string, updatePaymentMethodDto: UpdateCardInput) {
  ...
  saveDefaultCart(method_id: string) {
    ...
    return this.findOne(String(method_id));
  }
